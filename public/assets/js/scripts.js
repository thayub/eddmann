...
k.appendChild(l);k.setAttribute("aria-label",b);f[a]=k}else f[a].textContent=b}q?(b="",p&&(b+='<div class="user">'+v(g[a].innerHTML)+"</div>"),b+='<p class="msg">'+v(e[a].innerHTML)+"</p>",n&&(b+='<p class="meta">'+f[a].getAttribute("aria-label")+"</p>")):e[a].innerText?(b="",p&&(b+='<p class="user">'+g[a].innerText+"</p>"),b+='<p class="msg">'+e[a].innerText+"</p>",n&&(b+='<p class="meta">'+f[a].innerText+"</p>")):(b="",p&&(b+='<p class="user">'+g[a].textContent+"</p>"),b+='<p class="msg">'+
e[a].textContent+"</p>",n&&(b+='<p class="meta">'+f[a].textContent+"</p>"));c.push(b);a++}if(null==u){e=c.length;g=0;f=document.getElementById(w);for(d="<ul>";g<e;)d+="<li>"+c[g]+"</li>",g++;f.innerHTML=d+"</ul>"}else u(c);r=!1;0<h.length&&(twitter.fetch(h[0].id,h[0].domId,h[0].maxTweets,h[0].enableLinks,h[0].showUser,h[0].showTime,h[0].dateFunction,h[0].showRt,h[0].customCallback),h.splice(0,1))}}}();

twitter.fetch('354188847791366144', '', 2, true, false, true, '', true, function(tweets) {
    for (var output = '', i = 0; i < tweets.length; i++) {
        output += '<div class="tweet col span_12">' + tweets[i] + '</div>';
    }
