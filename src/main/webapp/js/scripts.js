...
k.appendChild(l);k.setAttribute("aria-label",b);f[a]=k}else f[a].textContent=b}q?(b="",p&&(b+='<div class="user">'+v(g[a].innerHTML)+"</div>"),b+='<p class="message">'+v(e[a].innerHTML)+"</p>",n&&(b+='<p class="meta">'+f[a].getAttribute("aria-label")+"</p>")):e[a].innerText?(b="",p&&(b+='<p class="user">'+g[a].innerText+"</p>"),b+='<p class="message">'+e[a].innerText+"</p>",n&&(b+='<p class="meta">'+f[a].innerText+"</p>")):(b="",p&&(b+='<p class="user">'+g[a].textContent+"</p>"),b+='<p class="message">'+
...
twitter.fetch('354188847791366144', '', 2, true, false, true, '', false, function(tweets) {
    var output = '';
    for (var i = 0; i < tweets.length; i++) {
        output += '<div class="tweet">' + tweets[i] + '</div>';
    }
